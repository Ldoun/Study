...
def load_dataset(data_transform, dataset_dir='./dataset',dataset_name='catdog',is_train=True):
    dataset_dir = os.path.join(dataset_dir,dataset_name)
    is_train_dir = 'train' if is_train else 'test'

    dataset = ImageFolder(
        os.path.join(dataset_dir,is_train_dir),
        data_transform[is_train_dir],
    )
...
def get_loaders(config,input_size):
    ...
    train_set = load_dataset(
        data_transform,dataset_name=dataset_name, is_train=True
    )
    test_set = load_dataset(
        data_transform, dataset_name=dataset_name, is_train=False
    )

    train_set, valid_set, test_set = divide_dataset(
        train_set,test_set,config.dataset_name,config.train_ratio,config.valid_ratio,config.test_ratio
    )
