        ...
        if not self.conv1d:
            ...
            for window_size, n_filter in zip(window_sizes, n_filters):
                self.feature_extractors.append(
                    nn.Sequential(
                        nn.Conv1d(
                            in_channels=word_vec_size,
                            out_channels=n_filter,
                            kernel_size=window_size
                        ),
                        nn.ReLU(),
                        nn.BatchNorm1d(n_filter) if use_batch_norm else nn.Dropout(dropout_p)
                    )
                )
    ...
    def forward(self, x):
        ...
        if not self.conv1d:
            ...
            if min_length > x.size(2):
                x = nn.functional.pad(x,(0,min_length-x.size(2)))
            
              
        
        # |x| = (batch_size, 1, length, word_vec_size)

        ## |x| = (batch_size, min_length, word_vec_size) because it's conv1d
        ...
        for block in self.feature_extractors:
            ...
            cnn_out = nn.functional.max_pool1d(
                input=cnn_out.squeeze(-1) if not self.conv1d else cnn_out,
                kernel_size=cnn_out.size(-2) if not self.conv1d else cnn_out.size(-1)
            ).squeeze(-1)
